        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
          {AVATAR_OPTIONS.map((avatar, index) => {
            ...
            return (
              <div 
                key={index}
                className={`relative aspect-square w-full max-w-[80px] cursor-pointer rounded-full p-1 transition-all
                  ${
                    currentAvatar === fullAvatarPath
                      ? 'border-2 border-primary-200 bg-primary-200/20'
                      : 'border border-gray-700 hover:border-primary-200/50'
                  }`}
