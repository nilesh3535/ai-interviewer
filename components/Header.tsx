...
export default function Header({ theme, setTheme }: ThemeSwitcherProps) {
  return (
    <nav className="navbar px-4 sm:px-20" role="navigation">
      <div className="flex-1 flex items-center">
        <Link href="/jobs" className="btn btn-ghost">
          <Image src="/jb.png" alt="AI Avatar" width={110} height={110} />
        </Link>
      </div>
      <div className="flex-none gap-4 flex items-center">
        <div className="hidden sm:flex gap-4">
          <ThemeSwitcher theme={theme} setTheme={setTheme} />
          <Link href="/" className="btn btn-ghost">AI Mock Interview</Link>
          <button
            className="btn btn-ghost"
            onClick={() => toast.info("AI Resume Builder is under development!", { duration: 2000 })}
          >
            AI Resume Builder
          </button>
        </div>
