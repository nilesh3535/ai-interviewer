...
// import Lottie, { LottieComponentProps } from "lottie-react"; // <--- REMOVE THIS
...
function ThemeSwitcher({ theme, setTheme }: ThemeSwitcherProps) {
  ...
  const ThemeIcon = () => (
    ...
    >
      ...
      <path d="M16.9109 13.7703H16.925C17.2062 13.7703 17.4734 13.6625 17.6703 13.4609C17.8718 13.2594 17.9796 12.9969 17.9796 12.7156C17.9796 12.4344 17.8671 12.1578 17.6656 11.9609L6.82339 1.2078C5.87651 0.256237 4.32964 0.251549 3.37808 1.20311L3.13433 1.44686C2.18277 2.39374 2.17808 3.94061 3.12964 4.89217L5.07964 6.84218L1.10933 10.8078C0.720265 11.1969 0.504640 11.7125 0.499953 12.2656C0.495265 12.8187 0.710890 13.3344 1.09995 13.7234C1.10933 13.7328 1.11870 13.7422 1.13277 13.7562L7.28277 19.6625C7.68589 20.0469 8.19683 20.239 8.71245 20.239C9.22808 20.239 9.74839 20.0422 10.1468 19.6578L10.8078 19.0203C12.7625 17.1312 15.7062 14.2812 16.0906 13.8969C16.114 13.8734 16.3062 13.7703 16.9109 13.7703ZM4.18901 3.83749C3.82339 3.46717 3.82339 2.87655 4.18901 2.51092L4.43277 2.26717C4.79839 1.90155 5.39370 1.90155 5.75933 2.27186L7.71402 4.21249L6.13433 5.79217L4.18901 3.83749ZM15.0359 12.8234C14.689 13.1703 11.6093 16.1515 9.76714 17.9375L9.10620 18.575C8.88589 18.7859 8.54370 18.7859 8.32339 18.5797L2.17339 12.6734L2.16402 12.6641C2.05620 12.5562 1.99995 12.4156 1.99995 12.2656C1.99995 12.1156 2.06089 11.975 2.16870 11.8672L6.13902 7.90624L8.78277 5.26249L15.9406 12.3641C15.5562 12.4531 15.2562 12.6031 15.0359 12.8234Z"></path>
    </svg>
  );
...
export default function JobApp() {
 ...
 const [animationData, setAnimationData] = useState<LottieComponentProps["animationData"] | null>(null);
      ...
      if (currentUser) {
        try {
          const response = await fetch(
            `https://kxiqztfueasspcdjpbfq.supabase.co/rest/v1/jobs?candidate_id=eq.${currentUser.id}&order=id.asc`,
            {
              headers: {
                apikey:
  ...
  useEffect(() => {
    ...
    handleResize();
    // Corrected cleanup: should remove 'resize' listener
        ...
        <div className="relative flex lg:flex-row w-full h-screen justify-center flex-col sm:p-0">
          <div className="lg:w-1/2 w-full h-full lg:grid items-center hidden">
            <div className="relative items-center justify-center flex z-1 flex-col gap-4">
              {/* Use ClientLottie here */}
              <ClientLottie animationData={animationData} loop autoplay className="w-48 h-48" />
            </div>
          </div>
          ...
          <div className="max-w-[640px] w-full mx-auto">
            ...
            {isOpen && (
              <div className="fixed inset-0 flex items-center justify-center overflow-y-auto z-[99999]">
                ...
                {/* Modal Content */}
                <div
                  className={`${isFullscreen ? "w-full h-full" : "relative w-full max-w-[700px] m-4 rounded-3xl bg-white dark:bg-gray-900"}`}
